...
class Personal extends AUTH_Controller { //Pegawai
	...
	public function index() {
		...
		$data['datapersonal'] = $this->M_personal->select_all();
		$data['dataPosisi'] = $this->M_posisi->select_all();
		$data['dataKota'] = $this->M_kota->select_all();

		$data['page'] = "personal";
		$data['cabTitulo'] = "Agentes";
		$data['cabDescri'] = "Administrar personal";

		$data['modal_personal'] = show_my_modal('modals/modal_personal', 'modal-personal', $data);

		$this->template->views('personal/home', $data);
	}

	public function listado() {
		$data['datapersonal'] = $this->M_personal->select_all();
		$this->load->view('personal/list_data', $data);
	}
	...
	public function procesaInsert() {
		...
		if ($this->form_validation->run() == TRUE) {
			$result = $this->M_personal->insert($data);
	...
	public function update() {
		...
		$data['datapersonal'] = $this->M_personal->select_by_id($id);
		$data['dataPosisi'] = $this->M_posisi->select_all();
		$data['dataKota'] = $this->M_kota->select_all();
		$data['userdata'] = $this->userdata;
		//esto es respuesta ajax
		echo show_my_modal('modals/modal_update_personal', 'update-personal', $data);
	}
	...
	public function procesaUpdate() {
		...
		$data = $this->input->post();
		if ($this->form_validation->run() == TRUE) {
			$result = $this->M_personal->update($data);

			if ($result > 0) {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('Data personal Berhasil diupdate', '20px');
			} else {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('Data personal Gagal diupdate', '20px');
			}
		} else {
			$out['status'] = 'form';
			$out['msg'] = show_err_msg(validation_errors());
	...
	public function delete() {
		$id = $_POST['id'];
		$result = $this->M_personal->delete($id);
	...
	public function export() {
		...
		$data = $this->M_personal->select_all_personal();
		...
		$objWriter->save('./assets/excel/Data personal.xlsx'); 

		$this->load->helper('download');
		force_download('./assets/excel/Data personal.xlsx', NULL);
	}
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				foreach ($sheetData as $key => $value) {
					if ($key != 1) {
						...
						$check = $this->M_personal->check_nama($value['B']);
				...
				if (count($resultData) != 0) {
					$result = $this->M_personal->insert_batch($resultData);
					if ($result > 0) {
						$this->session->set_flashdata('msg', show_succ_msg('Data personal Berhasil diimport ke database'));
						redirect('personal');
					}
				} else {
					$this->session->set_flashdata('msg', show_msg('Data personal Gagal diimport ke database (Data Sudah terupdate)', 'warning', 'fa-warning'));
					redirect('personal');
				}
